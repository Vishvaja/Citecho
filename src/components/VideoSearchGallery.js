...
function VideoSearchGallery({ searchQuery }) {
    ...
    useEffect(() => {
        ...
        const filteredVideos = videoUrls.filter(video =>
          ...
          video.authors.toLowerCase().includes(searchQuery.toLowerCase()) ||
          video.contentType.toLowerCase().includes(searchQuery.toLowerCase()) ||
          video.publisher.toLowerCase().includes(searchQuery.toLowerCase())
        ).map(video => ({ ...video, liked: false }));
...
export default VideoSearchGallery;
